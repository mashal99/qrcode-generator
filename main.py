...
DEFAULT_URL = os.getenv('QR_DATA_URL', 'https://github.com/mashal99')  # Default URL for QR code
...
def create_directory(path: Path):
    try:
        ...
        logging.info(f"Directory created or already exists at: {path}")
...
def generate_qr_code(data, path, fill_color='red', back_color='white'):
    ...
    try:
        ...
        # Log the intended file path for saving
        logging.info(f"Saving QR code to {path}")

        # Check if the directory for the path exists
        if not path.parent.exists():
            logging.error(f"Directory does not exist: {path.parent}")
            return
